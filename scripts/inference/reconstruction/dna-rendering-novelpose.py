#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DNA-Rendering novel-pose benchmark: T=0 canonical once, animate all frames.
...
Requires untarred data under ``evaluation/reconstruction_benchmark/``.

Example::

    python scripts/inference/reconstruction/dna-rendering-novelpose.py --debug
    python scripts/inference/reconstruction/dna-rendering-novelpose.py \\
...
_BENCHMARK_MODE = "novel_pose"


def get_parse() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="DNA LHMPP novel-pose benchmark inference")
    add_dna_benchmark_args(p)
    return p.parse_args()


@torch.no_grad()
def run_novel_pose_inference(
    lhm: torch.nn.Module,
    dataset,
    output_root: str,
    infer_cfg,
    ...
    *,
    ...
    if gs_export_enabled(cfg_train):
        _, _, _, patch_fn = _motion_helpers()
        patch_fn(lhm)
    ...
    _, build_cache_fn, _, _ = _motion_helpers()
    use_pred_shape = not BENCHMARK_USE_DATA_SHAPE
    ...
    for case_idx in range(bins * gpus, min(bins * (gpus + 1), n_cases)):
        ...
        forward_cache = None
        try:
            item0 = dataset.sample(case_idx, 0)
            motion0 = build_multiview_motion_pack(
                item0["target_cam_dirs"], int(item0["tgt_frame_id"]), infer_cfg
            )
            forward_cache = build_cache_fn(
                lhm,
                dict(item0),
                motion0,
                device="cuda",
            ...
            export_scene_reference_strip(output_root, item0)
            _LOG.info(
                "forward_cache built scene=%s src_frame=%s",
                scene_id,
                item0["src_frame_id"],
            )
        except Exception as ex:
            print(f"skip case={case_idx} forward_cache: {ex}", file=sys.stderr)
            traceback.print_exc()
            continue
        ...
        for t in range(ta, tb):
            ...
            try:
                motion_t = build_multiview_motion_pack(
                ...
                traceback.print_exc()
                _LOG.exception("novel-pose failed scene=%s t=%s", scene_id, t)
...
def main() -> None:
    ...
    if args.debug:
        ...
        try:
            ...
            print(
                f"sample(0,0) OK uid={item['uid']} src={item['src_frame_id']} "
                f"tgt={item['tgt_frame_id']} test_cams={n_test}"
            )
